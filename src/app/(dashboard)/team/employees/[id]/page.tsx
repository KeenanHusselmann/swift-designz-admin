              ...
              {employee.email && (
                <div>
                  ...
                  <dd className="text-sm text-white">
                    <a href={`mailto:${employee.email}`} className="hover:text-[#30B0B0] transition-colors">{employee.email}</a>
                  </dd>
                </div>
              )}
              {employee.phone && (
                <div>
                  ...
                  <dd className="text-sm text-white">{employee.phone}</dd>
                </div>
              )}
              <div>
                <dt className="text-xs text-gray-500 mb-1">Department</dt>
                <dd className="text-sm text-white">{departmentLabels[employee.department] || employee.department}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-500 mb-1">Role</dt>
                <dd className="text-sm text-white">{employee.role}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-500 mb-1">Start Date</dt>
                <dd className="text-sm text-white">{formatDate(employee.start_date)}</dd>
              </div>
              {employee.end_date && (
                <div>
                  <dt className="text-xs text-gray-500 mb-1">End Date</dt>
                  <dd className="text-sm text-white">{formatDate(employee.end_date)}</dd>
                </div>
              )}
              <div>
          ...
          <div className="glass-card overflow-hidden">
            <div className="px-6 py-4 border-b border-[#2a2a2a]">
              <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                <TrendingUp className="h-3.5 w-3.5 text-[#30B0B0]" />
                Salary History
              </h2>
            </div>
                ...
                <tbody className="divide-y divide-[#2a2a2a]">
                  {salaryHistory.map((h) => (
                    <tr key={h.id} className="hover:bg-[#1a1a1a] transition-colors">
                      ...
                      <td className="px-6 py-3 text-sm text-white font-medium text-right">{formatCurrency(h.amount)}/mo</td>
          ...
          <div className="glass-card p-6">
            ...
            <dl className="space-y-4">
              <div>
                ...
                <dd className="text-lg font-semibold text-white">{formatCurrency(employee.salary)}/mo</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-500 mb-1">Annual</dt>
                <dd className="text-lg font-semibold text-[#30B0B0]">{formatCurrency(employee.salary * 12)}/yr</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-500 mb-1">Salary Changes</dt>
                <dd className="text-lg font-semibold text-white">{salaryHistory.length}</dd>
              </div>
            </dl>
          </div>
        </div>
