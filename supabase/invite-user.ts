/**
 * Invite an INVESTOR to the admin portal.
 * Generates a password-setup link via Supabase admin API and sends a branded
 * email via Resend — bypasses Supabase email templates and Site URL settings.
 *
 * For employee/staff invites (magic link, no password), use:
 *   supabase/invite-employee.ts
 *
 * Usage:
 *   npx tsx supabase/invite-user.ts
 */
...
const FULL_NAME = "Investor Name";
const ROLE: "admin" | "viewer" | "investor" = "investor";
// ────────────────────────────────────────────────────────────────────────────

const PORTAL_URL = "https://admin.swiftdesignz.co.za";

async function main() {
  console.log(`Inviting ${EMAIL} as role="${ROLE}"...`);

  // 1. Create auth user (email confirmed so they can set password via link)
  ...
  // 3. Generate a password-reset/recovery link (acts as first-time password setup)
  ...
  if (emailError) {
    ...
    console.log("Invite link (send manually):", inviteLink);
    process.exit(1);
  }
        ...
        <table width="580" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:580px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;">
              ...
              <h1 style="margin:0 0 24px;font-size:22px;font-weight:700;color:#fff;line-height:1.3;">Set Up Your Account</h1>
              ...
              <table cellpadding="0" cellspacing="0" style="margin:24px 0;">
                <tr>
                  <td style="background:#30B0B0;border-radius:8px;">
                    <a href="${link}" target="_blank" style="display:inline-block;padding:14px 32px;font-size:14px;font-weight:700;color:#fff;text-decoration:none;letter-spacing:0.5px;">Activate Account</a>
                  </td>
                </tr>
              </table>
